...
# Define holiday periods (Eid weeks and other holidays)
holiday_periods = [
...
def load_data():
    ...
    # Interaction term for Eid
    df['demandMW_lag1_eid'] = df['demandMW_lag1'] * df['is_eid']
...
# Plot historical Eid demand for reference
...
# Features
features = ['demandMW_lag1', 'hour_sin', 'hour_cos', 'is_holiday', 'is_eid', 'temp', 'demandMW_lag1_eid']
X_train = train_df[features]
y_train = train_df['demandMW']

# Sample weights: Increase weight for Eid samples
sample_weights = np.where(train_df['is_eid'] == 1, 10, 1)
        ...
        if len(feature_data) < 1:
            ...
            if last_known_date < user_date:
                ...
                for dt in pred_range:
                    ...
                    is_holiday, is_eid = is_holiday_date(dt, holiday_periods, annual_holidays, variable_holidays)
                    temp = temp_avg.get((dt.month, dt.hour), 25.0)
                    demand_lag1_eid = demand_lag1 * is_eid
                    
                    features_array = np.array([[demand_lag1, hour_sin, hour_cos, is_holiday, is_eid, temp, demand_lag1_eid]])
                    ...
                    new_row = pd.DataFrame({
                        ...
                        'is_eid': [is_eid],
                        'temp': [temp],
                        'demandMW_lag1_eid': [demand_lag1_eid]
                    })
