...
# Estimate average temperature by month and hour for future predictions
...
# Features
features = ['demandMW_lag1', 'hour_sin', 'is_holiday', 'is_eid', 'temp']
        ...
        if len(feature_data) < 1:
            ...
            if last_known_date < user_date:
                ...
                for dt in pred_range:
                    ...
                    temp = temp_avg.get((dt.month, dt.hour), 25.0)  # Default to 25°C if not found
                    
                    features_array = np.array([[demand_lag1, hour_sin, is_holiday, is_eid, temp]])
                    pred = rf_final.predict(features_array)[0]
                    
                    if is_eid:
                        pred = min(max(pred, 500), 700)
