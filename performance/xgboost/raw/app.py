...
# Define holiday periods (Eid weeks only, removing other holidays)
holiday_periods = [
...
train_df = pd.concat([train_df, eid_train_df, eid_train_df], ignore_index=True)  # 3x Eid data
...
# Sample weights
sample_weights = np.where(train_df['is_eid'] == 1, 50, 1)
...
if user_date:
    ...
    if user_date < pd.to_datetime('2025-03-11') or user_date > pd.to_datetime('2030-12-31'):
        ...
        if len(feature_data) < 1:
            ...
            # Validation plot for historical Eid (April 10, 2024)
